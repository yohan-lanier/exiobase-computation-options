...
def extract_exiobase_data(version: str, reference_year: str) -> ExiobaseRelevantData:
    ...
    s_file_path = folder_path / f"S_{reference_year}.csv"
    a_file_path = folder_path / f"A_{reference_year}.csv"
    units_file_path = folder_path / f"units_{reference_year}.csv"
    if s_file_path.exists() and a_file_path.exists() and units_file_path.exists():
        return open_cached_data(
    ...
    exiobase_light_data = get_relevant_exiobase_data(exiobase, version, reference_year)
...
class CachedDataPaths(TypedDict):
    a: Path
    s: Path
    units: Path


def open_cached_data(
    file_paths: CachedDataPaths, reference_year: str
    ...
    return {
        "a": pd.read_csv(
            file_paths["a"], sep=",", low_memory=False, header=[0, 1], index_col=[0, 1]
        ),
        "s": pd.read_csv(
            file_paths["s"], sep=",", low_memory=False, header=[0, 1], index_col=[0]
        ),
        "units": pd.read_csv(
            file_paths["units"], sep=",", low_memory=False, index_col=[0]
        ),
...
def cache_useful_data(
    exiobase_light_data: ExiobaseRelevantData, file_paths: CachedDataPaths
) -> None:
